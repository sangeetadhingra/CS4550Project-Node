...
const API_BASE = process.env.REACT_APP_API_BASE ? process.env.REACT_APP_API_BASE : "http://localhost:4000/api";
  ...
  const searchRecipeByID = async (req, res) => {
    ...
    const RECIPE_UR = API_URL.replace("{$}", id);
    const response = (await Axios.get(RECIPE_UR));
